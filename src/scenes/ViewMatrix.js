class ViewMatrix extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(config.width / 2, config.height * 0.95, "Your team's moves so far are listed above, with each row representing one turn and each column representing its stated resource. The first part of an entry is the absolute quantity of that resource purchased on that turn, while the second part is the percentage of the turn's budget spent on that resource.\n\nRemember - one thousand (1,000) total consumption purchased guarantees a win!").setOrigin(0.5, 1).setFontSize(18).setWordWrapWidth(config.width * 0.9).setAlign('center');
